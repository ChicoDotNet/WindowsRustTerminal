...
namespace r09
{
    ...
    inline bool output_csi_push_sgr_replay()
    {
        ...
        {
            std::fprintf(stderr, "output CSI push SGR sizing mismatch: status=%u required=%zu matched=%u\n", static_cast<unsigned>(status), required, matched);
            return false;
        }
        ...
        {
            std::fprintf(stderr, "output CSI push SGR primary payload mismatch: status=%u required=%zu matched=%u value=%d\n", static_cast<unsigned>(status), required, matched, singleOutput);
            return false;
        }
        ...
        status = terminal_parser_ffi_output_csi_push_sgr_values(
            ...
            &required,
            &matched);
        if (status != TERMINAL_PARSER_FFI_BUFFER_TOO_SMALL || required != 2 || matched != 1)
        {
            std::fprintf(stderr, "output CSI push SGR alias capacity mismatch: status=%u required=%zu matched=%u\n", static_cast<unsigned>(status), required, matched);
            return false;
        }
        ...
        status = terminal_parser_ffi_output_csi_push_sgr_values(
            ...
            &required,
            &matched);
        if (status != TERMINAL_PARSER_FFI_OK || required != 2 || matched != 1 || multipleOutput[0] != 1 || multipleOutput[1] != 2)
        {
            std::fprintf(
                stderr,
                "output CSI push SGR alias payload mismatch: status=%u required=%zu matched=%u values=%d,%d\n",
        ...
        {
            std::fprintf(stderr, "output CSI unrelated push SGR mismatch: status=%u required=%zu matched=%u\n", static_cast<unsigned>(status), required, matched);
            return false;
        }
        ...
        if (status != TERMINAL_PARSER_FFI_INVALID_ARGUMENT)
        {
            std::fprintf(stderr, "output CSI push SGR null matched mismatch: status=%u\n", static_cast<unsigned>(status));
            return false;
        }
