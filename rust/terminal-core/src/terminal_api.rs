...
impl TerminalApiState {
    #[must_use]
    pub fn colors(&self) -> &ColorTableState { &self.colors }

    pub fn set_color_table_entry(&mut self, index: usize, color: Rgb) -> bool {
        if index > u8::MAX as usize { return false; }
        let payload = format!("{index};rgb:{:02x}/{:02x}/{:02x}", color.r, color.g, color.b);
        self.colors.apply_osc(4, &payload)
    }

    #[must_use]
    pub const fn cursor(&self) -> CursorState { self.cursor }
    pub const fn set_cursor_blinking(&mut self, blinking: bool) { self.cursor.blinking = blinking; }
    pub const fn set_cursor_visible(&mut self, visible: bool) { self.cursor.visible = visible; }
    ...
    pub const fn close_hyperlink(&mut self) { self.current_hyperlink = None; }
    #[must_use]
    pub const fn current_hyperlink_id(&self) -> Option<u16> { self.current_hyperlink }
    #[must_use]
    pub fn hyperlink_uri(&self, id: u16) -> Option<&str> { self.hyperlinks.uri(id) }

    pub fn write_utf16(&mut self, text: &[u16]) -> usize {
        let before = self.written_text.chars().count();
        self.written_text.extend(char::decode_utf16(text.iter().copied()).map(|decoded| decoded.unwrap_or(char::REPLACEMENT_CHARACTER)));
        self.written_text.chars().count() - before
    }
    #[must_use]
    pub fn written_text(&self) -> &str { &self.written_text }
    #[must_use]
    pub const fn taskbar_state(&self) -> u8 { self.taskbar_state }
    #[must_use]
    pub const fn taskbar_progress(&self) -> u8 { self.taskbar_progress }

    pub fn set_taskbar_progress(&mut self, state: Option<u8>, progress: Option<u16>) -> bool {
        let state = state.unwrap_or(0);
        if state > 4 { return false; }
        match state {
            0 => { self.taskbar_state = 0; self.taskbar_progress = 0; }
            3 => { self.taskbar_state = 3; }
            _ => {
                self.taskbar_state = state;
                let progress = progress.map_or(1, |value| value.min(100));
                self.taskbar_progress = u8::try_from(progress).expect("progress is clamped to 100");
            }
        }
        true
    }
    ...
    pub fn set_working_directory(&mut self, payload: &str) -> bool {
        let value = if let Some(inner) = payload.strip_prefix('"').and_then(|s| s.strip_suffix('"')) {
            if inner.is_empty() || inner.contains('"') { return false; }
            inner
        } else {
            if payload.is_empty() || payload.contains('"') { return false; }
            payload
        };
...
mod tests {
    ...
    #[test]
    fn microsoft_terminal_api_hyperlink_contracts() {
        let mut terminal = TerminalApiState::default();
