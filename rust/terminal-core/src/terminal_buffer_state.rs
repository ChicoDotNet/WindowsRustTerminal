...
impl TerminalBufferState {
    pub fn new(width: u16, viewport_height: u16, history_rows: u16) -> Result<Self, TextBufferError> {
        let total_height = viewport_height.saturating_add(history_rows).max(1);
        Ok(Self { buffer: TextBuffer::new(width, total_height, TextAttribute::default())?, viewport_height: viewport_height.max(1), history_rows, mutable_viewport_top: 0, scroll_offset: 0, line_feeds: 0 })
    }

    #[must_use]
    pub const fn buffer(&self) -> &TextBuffer { &self.buffer }
    #[must_use]
    pub const fn viewport_top(&self) -> u16 { self.mutable_viewport_top.saturating_sub(self.scroll_offset) }
    #[must_use]
    pub const fn viewport_bottom_exclusive(&self) -> u16 { self.viewport_top().saturating_add(self.viewport_height) }
    #[must_use]
    pub const fn scroll_offset(&self) -> u16 { self.scroll_offset }
    ...
    pub fn line_feed(&mut self) {
        ...
        if self.line_feeds < u32::from(self.viewport_height) { return; }
        if self.mutable_viewport_top < self.history_rows {
            self.mutable_viewport_top += 1;
            if self.scroll_offset > 0 { self.scroll_offset = self.scroll_offset.saturating_add(1).min(self.history_rows); }
...
mod tests {
    ...
    fn microsoft_terminal_buffer_simple_writing_contract() {
        ...
        terminal.write_text_at(0, 0, &utf16("Hello World")).expect("source text fits");
        assert_eq!(terminal.viewport_top(), 0);
        assert_eq!(terminal.viewport_bottom_exclusive(), 32);
        assert_eq!(terminal.buffer().row(0).glyph_at(0), &[u16::from(b'H')]);
        assert_eq!(terminal.buffer().row(0).glyph_at(10), &[u16::from(b'd')]);
    }
    ...
    fn microsoft_terminal_buffer_dont_snap_to_output_contract() {
        ...
        for _ in 0..(u32::from(HEIGHT) + 8 - 1) { terminal.line_feed(); }
        assert_eq!(terminal.viewport_top(), 8);
        terminal.set_scroll_offset(1);
        for _ in 0..8 { terminal.line_feed(); }
        assert_eq!(terminal.viewport_top(), 7);
        assert_eq!(terminal.scroll_offset(), 9);
        while terminal.mutable_viewport_top < HISTORY { terminal.line_feed(); }
        for _ in 0..3 { terminal.line_feed(); }
