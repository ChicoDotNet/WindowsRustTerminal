...
fn reject_everything(settings: &mut MediaResourceSettings) {
...
fn microsoft_media_resource_validate_resolver_called_for_inbox_contract() {
    ...
    let background = settings.profile_background("Base", false).unwrap();
    assert!(background.ok);
    assert_eq!(background.resolved, "resolved");
}
...
fn microsoft_media_resource_validate_resolver_called_for_inbox_and_user_contract() {
    let mut settings = settings(
        r#"{
            "profiles": {
                "defaults": { "icon": "iconFromDefaults" },
                "list": [{
            ...
            "actions": [{
                "command": { "action": "sendInput", "input": "IT CAME FROM BEYOND THE STARS" },
                "icon": null,
                "id": "Terminal.CloseWindow"
            }]
        }"#,
...
fn microsoft_media_resource_validate_resolver_called_for_fragments_contract() {
    let fragment = MediaFragment {
        ...
        content: r#"{
            ...
            "actions": [{
                "command": { "action": "sendInput", "input": "SOME DAY SOMETHING'S COMING" },
                "icon": "foo.ico",
                "id": "Dustin.SendInput"
            }]
        }"#,
...
fn microsoft_media_resource_validate_resolver_called_for_new_tab_menu_entries_contract() {
    let mut settings = settings(
        r#"{
            "newTabMenu": [
                { "icon": "menuItemIcon1", "id": "Terminal.CloseWindow", "type": "action" },
                { "icon": "menuItemIcon2", "profile": "{862d46aa-cc9c-4e6c-b872-9cadaafcdbbe}", "type": "profile" },
                {
                    "allowEmpty": true,
                    "entries": [
                        { "icon": "menuItemIcon4", "profile": "{862d46aa-cc9c-4e6c-b872-9cadaafcdbbe}", "type": "profile" },
                        {
                            "allowEmpty": true,
                            "entries": [
                                { "icon": "menuItemIcon6", "profile": "{862d46aa-cc9c-4e6c-b872-9cadaafcdbbe}", "type": "profile" }
                            ],
                            "icon": "menuItemIcon5",
                            "inline": "never",
                            "name": "Or was it...?",
...
fn microsoft_media_resource_validate_resolver_called_incrementally_on_change_contract() {
    ...
    assert_eq!(calls, 1);
    let resolved = settings.profile_icon("Base").unwrap();
    assert!(resolved.ok);
    assert_eq!(resolved.path, "NewIconFromRuntime");
    assert_eq!(resolved.resolved, "newResolvedValue");
}
...
fn microsoft_media_resource_validate_resolver_not_called_for_emoji_icons_contract() {
    let mut settings = settings(
        r#"{
            "profiles": { "list": [
                { "icon": "♥", "name": "Basic" },
                { "icon": "", "name": "MDL2" },
                { "icon": "👨‍👩‍👧‍👦", "name": "GraphemeCluster" },
                { "icon": "🕴️", "name": "SurrogatePair" },
                { "icon": "#️⃣", "name": "VariantWithEnclosingCombiner" }
            ] }
        }"#,
        &[],
    );
    ...
    assert_eq!(origins.get(&MediaOrigin::InBox), Some(&11));
    assert_eq!(origins.get(&MediaOrigin::User), None);
...
fn microsoft_media_resource_profile_defaults_contains_invalid_icon_contract() {
    ...
    reject_everything(&mut settings);
    let icon = settings.profile_icon("Base").unwrap();
    assert!(icon.ok);
    assert_eq!(icon.resolved, CMD);
}
...
fn microsoft_media_resource_profile_specifies_invalid_icon_and_commandline_contract() {
    let mut settings = settings(
        r#"{
            "profiles": {
                "defaults": { "icon": "DoesNotMatter", "commandline": "C:\\Windows\\System32\\ping.exe" },
                "list": [{
                    "guid": "{2cdb0be2-f601-4f70-9a6c-3472b3257883}",
                    "icon": "DoesNotMatter",
    ...
    reject_everything(&mut settings);
    let icon = settings.profile_icon("ProfileSpecifiesInvalidIconAndCommandline").unwrap();
    assert!(icon.ok);
    assert_eq!(icon.resolved, CSCRIPT);
}
...
fn microsoft_media_resource_profile_overwrites_bell_sound_contract() {
    let mut settings = settings(
        r#"{
            "profiles": { "list": [{
                "guid": "{862d46aa-cc9c-4e6c-b872-9cadaafcdbbe}",
                "bellSound": ["does not matter; resolved rejected"]
            }] }
        }"#,
        &[],
    );
...
fn microsoft_media_resource_fragment_action_resources_get_resolved_contract() {
    let fragment = MediaFragment {
        ...
        content: r#"{
            ...
            "actions": [{
                "command": { "action": "sendInput", "input": "FROM WAY OUT BEYOND THE STARS" },
                "icon": "foo.ico",
                "id": "Dustin.SendInput"
            }]
        }"#,
...
fn microsoft_media_resource_fragment_appearance_and_user_appearance_interaction_contract() {
            ...
            "profiles": [
                {
                    ...
                    "unfocusedAppearance": { "backgroundImage": "unfocusedBackgroundImage1" }
                },
                {
    ...
    let mut settings = settings(
        r#"{
            "profiles": [
                {
                    "guid": "{4e7c2b36-642f-4694-83f8-8a5052038a23}",
                    "unfocusedAppearance": { "experimental.pixelShaderPath": "unfocusedPixelShaderPath1" }
                },
                {
                    "guid": "{94df2990-d645-4675-8d9d-f8c89f842e6b}",
                    "unfocusedAppearance": { "backgroundImage": "userSpecifiedUnfocusedBackgroundImage" }
                }
            ]
        }"#,
        &[fragment],
    ...
    settings.resolve_media_resources(|_, base_path, resource| {
        let resolved = format!("{}-{}", base_path, resource.path());
        resource.resolve(&resolved);
    });
...
fn microsoft_media_resource_real_resolver_url_cases_contract() {
    ...
    assert_resolution(
        resolve_media_path("ms-appx://Microsoft.Burrito/Resources/explorer.exe", USER_BASE, &platform),
        true,
        r"C:\Windows\explorer.exe",
    );
    for uri in [
