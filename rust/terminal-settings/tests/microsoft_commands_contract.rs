...
fn microsoft_commands_split_pane_args_contract() {
    ...
    assert_eq!(commands.split_direction("command1"), Some(SplitDirection::Right));
    assert_eq!(commands.split_direction("command2"), Some(SplitDirection::Down));
    assert_eq!(commands.split_direction("command4"), Some(SplitDirection::Automatic));
    assert_eq!(commands.split_direction("command5"), Some(SplitDirection::Automatic));
    assert_eq!(commands.split_size("command6"), Some(0.25));
    assert_eq!(commands.split_direction("command7"), Some(SplitDirection::Right));
    assert_eq!(commands.split_direction("command8"), Some(SplitDirection::Left));
    assert_eq!(commands.split_direction("command9"), Some(SplitDirection::Up));
    assert_eq!(commands.split_direction("command10"), Some(SplitDirection::Down));
}
...
fn microsoft_commands_autogenerated_name_contract() {
    ...
    assert_eq!(commands.split_direction("Split pane"), Some(SplitDirection::Automatic));
    assert_eq!(commands.split_direction("Split pane, split: left"), Some(SplitDirection::Left));
    assert_eq!(commands.split_direction("Split pane, split: right"), Some(SplitDirection::Right));
    assert_eq!(commands.split_direction("Split pane, split: up"), Some(SplitDirection::Up));
    assert_eq!(commands.split_direction("Split pane, split: down"), Some(SplitDirection::Down));
}
...
fn microsoft_commands_layer_on_autogenerated_name_contract() {
    ...
    assert_eq!(commands.command_count(), 1);
    assert_eq!(commands.split_direction("Split pane"), Some(SplitDirection::Right));
}
...
fn microsoft_commands_generate_commandline_contract() {
    ...
    assert_eq!(commands.commandline("action1").as_deref(), Some(r#"--profile "foo""#));
    assert_eq!(commands.commandline("action2").as_deref(), Some(r#"--profile "foo""#));
    assert_eq!(commands.commandline("action3").as_deref(), Some(r#"-- "bar.exe""#));
    assert_eq!(commands.commandline("action4").as_deref(), Some(r#"-- "pop.exe ya ha ha""#));
    assert_eq!(commands.commandline("action5").as_deref(), Some(r#"-- "pop.exe "ya ha ha"""#));
    assert_eq!(commands.commandline("action6").as_deref(), Some(r#"--startingDirectory "C:\foo" -- "bar.exe""#));
    assert_eq!(
        commands
