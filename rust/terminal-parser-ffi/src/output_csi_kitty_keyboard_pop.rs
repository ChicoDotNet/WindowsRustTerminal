...
pub enum OutputCsiKittyKeyboardPopKind {
    None = 0,
    Pop = 1,
}
...
#[repr(C)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct OutputCsiKittyKeyboardPopPlan {
    pub kind: u32,
    pub count: i32,
    pub reserved0: u32,
    pub reserved1: u32,
...
impl Default for OutputCsiKittyKeyboardPopPlan {
    fn default() -> Self {
        Self {
            kind: OutputCsiKittyKeyboardPopKind::None as u32,
            count: 0,
            reserved0: 0,
            reserved1: 0,
        }
...
#[derive(Default)]
struct PlanDispatch {
    plan: OutputCsiKittyKeyboardPopPlan,
}
    ...
    fn dispatch(&mut self, action: OutputAction) {
        self.plan = match action {
            OutputAction::AdvancedCsi { id, parameters } if id == VtId::from_ascii("<u") => {
                OutputCsiKittyKeyboardPopPlan {
                    kind: OutputCsiKittyKeyboardPopKind::Pop as u32,
                    count: parameters.at(0).unwrap_or(0),
                    reserved0: 0,
                    reserved1: 0,
                }
            }
            _ => OutputCsiKittyKeyboardPopPlan::default(),
        };
    }
}
...
/// Replays Kitty keyboard pop classification and its effective count through
/// the Rust output engine. Native C++ retains only terminal-state materialization.
#[unsafe(no_mangle)]
pub extern "C" fn terminal_parser_ffi_output_csi_kitty_keyboard_pop_plan(
    identifier: u64,
    parameter0: i32,
    out_plan: *mut OutputCsiKittyKeyboardPopPlan,
) -> FfiStatus {
    ffi_guard(|| {
        if out_plan.is_null() {
...
mod tests {
    use super::{
        OutputCsiKittyKeyboardPopKind, OutputCsiKittyKeyboardPopPlan,
        terminal_parser_ffi_output_csi_kitty_keyboard_pop_plan,
    };
    use crate::FfiStatus;
    use terminal_parser::state_machine::VtId;

    fn expect(id: &str, parameter0: i32, kind: OutputCsiKittyKeyboardPopKind, count: i32) {
        let mut result = OutputCsiKittyKeyboardPopPlan::default();
        assert_eq!(
            terminal_parser_ffi_output_csi_kitty_keyboard_pop_plan(
    ...
    #[test]
    fn csi_kitty_keyboard_pop_ffi_replays_microsoft_contract() {
        expect("<u", 0, OutputCsiKittyKeyboardPopKind::Pop, 0);
        expect("<u", 1, OutputCsiKittyKeyboardPopKind::Pop, 1);
        expect("<u", 7, OutputCsiKittyKeyboardPopKind::Pop, 7);
        expect(">u", 7, OutputCsiKittyKeyboardPopKind::None, 0);
    }

    #[test]
    fn csi_kitty_keyboard_pop_ffi_validates_pointer_and_identifier() {
        assert_eq!(
            terminal_parser_ffi_output_csi_kitty_keyboard_pop_plan(
                0,
                0,
