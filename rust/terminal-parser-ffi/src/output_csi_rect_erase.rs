...
pub enum OutputCsiRectEraseKind {
    None = 0,
    Erase = 1,
    SelectiveErase = 2,
}
...
impl TermDispatch for PlanDispatch {
    fn dispatch(&mut self, action: OutputAction) {
        if let OutputAction::AdvancedCsi { id, parameters } = action {
            let kind = if id == VtId::from_ascii("$z") {
                OutputCsiRectEraseKind::Erase
            } else if id == VtId::from_ascii("${") {
                OutputCsiRectEraseKind::SelectiveErase
            } else {
                OutputCsiRectEraseKind::None
            };

            if kind != OutputCsiRectEraseKind::None {
...
/// Replays DECERA/DECSERA classification and their complete flat parameter
/// payload through the Rust output engine. Native dispatch retains rectangle
/// semantics while Rust owns portable VT classification and payload identity.
#[unsafe(no_mangle)]
pub extern "C" fn terminal_parser_ffi_output_csi_rect_erase_values(
    identifier: u64,
    values: *const i32,
    value_count: usize,
    out_values: *mut i32,
    ...
    ffi_guard(|| {
        ...
        // SAFETY: `out_count` and `out_kind` were checked non-null above.
        unsafe {
            ptr::write(out_count, required);
            ptr::write(out_kind, dispatch.kind);
        }
...
#[cfg(test)]
mod tests {
    use super::{OutputCsiRectEraseKind, terminal_parser_ffi_output_csi_rect_erase_values};
    use crate::FfiStatus;
    use terminal_parser::state_machine::VtId;

    fn replay(id: &str, values: &[i32]) -> (u32, Vec<i32>) {
        let mut required = 0usize;
        let mut kind = OutputCsiRectEraseKind::None as u32;
        let status = terminal_parser_ffi_output_csi_rect_erase_values(
        ...
        assert_eq!(
            terminal_parser_ffi_output_csi_rect_erase_values(
                VtId::from_ascii(id).value(),
                values.as_ptr(),
                values.len(),
                output.as_mut_ptr(),
    ...
    #[test]
    fn csi_rect_erase_ffi_replays_both_complete_flat_payloads() {
        assert_eq!(
            replay("$z", &[1, 2, 3, 4, 99]),
            (
                OutputCsiRectEraseKind::Erase as u32,
                vec![1, 2, 3, 4, 99]
            )
        );
        assert_eq!(
            replay("${", &[5, 6, 0, 0]),
            (
                OutputCsiRectEraseKind::SelectiveErase as u32,
                vec![5, 6, 0, 0]
            )
        );
        assert_eq!(
            replay("$x", &[1, 2, 3, 4]),
            (OutputCsiRectEraseKind::None as u32, Vec::new())
        );
    }

    #[test]
    fn csi_rect_erase_ffi_reports_capacity_and_validates_arguments() {
        let values = [1, 2, 3, 4];
        let mut required = 0usize;
        let mut kind = OutputCsiRectEraseKind::None as u32;
        let mut undersized = [0i32; 3];
        assert_eq!(
            terminal_parser_ffi_output_csi_rect_erase_values(
        ...
        assert_eq!(kind, OutputCsiRectEraseKind::Erase as u32);
        assert_eq!(
            terminal_parser_ffi_output_csi_rect_erase_values(
                VtId::from_ascii("$z").value(),
                std::ptr::null(),
