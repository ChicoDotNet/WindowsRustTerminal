...
impl TermDispatch for PlanDispatch {
    fn dispatch(&mut self, action: OutputAction) {
        if let OutputAction::SetCharacterProtectionAttribute(parameters) = action {
...
/// Replays CSI DECSCA classification and its complete flat parameter payload
...
#[unsafe(no_mangle)]
pub extern "C" fn terminal_parser_ffi_output_csi_decsca_values(
    identifier: u64,
    values: *const i32,
    value_count: usize,
    out_values: *mut i32,
...
#[cfg(test)]
mod tests {
    use super::terminal_parser_ffi_output_csi_decsca_values;
    use crate::FfiStatus;
    use terminal_parser::state_machine::VtId;

    fn replay(id: &str, values: &[i32]) -> Vec<i32> {
        let mut required = 0usize;
        let status = terminal_parser_ffi_output_csi_decsca_values(
        ...
        assert_eq!(
            terminal_parser_ffi_output_csi_decsca_values(
                VtId::from_ascii(id).value(),
                values.as_ptr(),
                values.len(),
                output.as_mut_ptr(),
    ...
    #[test]
    fn csi_decsca_ffi_replays_classification_and_parameter_payload() {
        assert_eq!(replay("\"q", &[0]), [0]);
        assert_eq!(replay("\"q", &[1, 2]), [1, 2]);
        assert!(replay("m", &[1]).is_empty());
    }

    #[test]
    fn csi_decsca_ffi_reports_capacity_and_validates_arguments() {
        let values = [1, 2];
        let mut required = 0usize;
        let mut one = 0i32;
        assert_eq!(
