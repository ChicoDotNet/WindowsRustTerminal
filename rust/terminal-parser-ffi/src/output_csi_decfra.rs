...
/// Replays DECFRA classification and its complete flat parameter payload
/// through the Rust output engine. DECFRA does not accept subparameters, so
/// caller-owned i32 arrays preserve the complete portable contract.
#[unsafe(no_mangle)]
pub extern "C" fn terminal_parser_ffi_output_csi_decfra_values(
    identifier: u64,
    values: *const i32,
    value_count: usize,
    out_values: *mut i32,
...
#[cfg(test)]
mod tests {
    use super::terminal_parser_ffi_output_csi_decfra_values;
    use crate::FfiStatus;
    ...
    fn replay(id: &str, values: &[i32]) -> (bool, Vec<i32>) {
        ...
        let status = terminal_parser_ffi_output_csi_decfra_values(
        ...
        assert_eq!(
            terminal_parser_ffi_output_csi_decfra_values(
                VtId::from_ascii(id).value(),
                values.as_ptr(),
                values.len(),
                output.as_mut_ptr(),
    ...
    #[test]
    fn csi_decfra_ffi_replays_complete_flat_payload() {
        assert_eq!(
            replay("$x", &[65, 1, 2, 3, 4]),
            (true, vec![65, 1, 2, 3, 4])
        );
        assert_eq!(replay("$r", &[1, 2, 3, 4, 5]), (false, Vec::new()));
    }

    #[test]
    fn csi_decfra_ffi_reports_capacity_and_validates_arguments() {
        let values = [65, 1, 2, 3, 4];
        let mut required = 0usize;
        let mut matched = 0u32;
        let mut undersized = [0i32; 4];
        assert_eq!(
            terminal_parser_ffi_output_csi_decfra_values(
