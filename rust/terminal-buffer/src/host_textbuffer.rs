...
//! ordinary host writes. This module closes the deterministic behavior that
//! crosses those owners: right-edge VT cursor motion, multi-row word boundaries,
//! selection rectangles, the full `CopyRequest` plain-text matrix, and command
//! mark remapping when text reflows to a different width.
...
/// Applies host CUB-style cursor motion while keeping the cursor inside the row.
#[must_use]
...
/// Applies the host newline result used after parsed VT output: column zero on
/// the next physical row, clamped to the represented buffer.
#[must_use]
pub fn next_line_clamped(
    cursor: TextBufferPoint,
    buffer_height: u16,
) -> TextBufferPoint {
    TextBufferPoint::new(
...
/// Returns the beginning of the word/delimiter run containing `point`.
///
/// Selection mode crosses physical rows only through forced-wrap links.
/// Accessibility mode treats the whole buffer as one row-major document and
/// associates whitespace with the word immediately to its left.
#[must_use]
...
/// Returns the exclusive end of the word/delimiter run containing `point`.
///
/// In accessibility mode the following whitespace is part of the current word;
/// the returned endpoint is therefore the beginning of the next word, or the
/// one-past-right endpoint of the final buffer row.
#[must_use]
pub fn get_word_end(
    buffer: &TextBuffer,
    point: TextBufferPoint,
...
/// Produces the physical rectangles used by selection and UIA consumers.
/// Endpoints are inclusive, with wide-glyph halves expanded exactly as the host
/// does before exposing the rectangles.
#[must_use]
pub fn get_text_rects(
    ...
    if block_selection {
        ...
        for y in start.y..=end.y {
            ...
            let row_left = row.adjust_to_glyph_start(i32::from(left));
            let row_right = expand_inclusive_right(row, right, buffer.width());
...
/// Implements the complete portable `TextBuffer::CopyRequest` formatting matrix.
#[must_use]
pub fn get_plain_text(buffer: &TextBuffer, request: PlainTextRequest) -> Vec<u16> {
    ...
    for y in start.y..=end.y {
        ...
        if request.include_crlf
            && y != end.y
            && (physical_rows || !row.was_wrap_forced())
        {
...
/// Reflows shell-integration mark coordinates from the source buffer width to a
/// destination width. The mapping is based on the buffer's hard-line/forced-wrap
/// topology, so marker coordinates move with their logical text rather than with
/// the old physical row numbers.
#[must_use]
...
fn previous_cell(
    ...
    if point.x > 0 {
        let x = row.navigate_to_previous(i32::from(point.x));
        return Some(TextBufferPoint::new(x, point.y));
    }
    if point.y == 0 {
        return None;
    }
    ...
    let x = previous_row.adjust_to_glyph_start(i32::from(buffer.width().saturating_sub(1)));
    Some(TextBufferPoint::new(x, previous_y))
}
...
fn next_cell(
    ...
    if point.y.saturating_add(1) >= buffer.height() {
        return None;
    }
    if !cross_unwrapped && !row.was_wrap_forced() {
        return None;
    }
...
fn rows_for_columns(columns: u32, width: u16) -> u32 {
    if columns == 0 {
        ...
        columns
            .saturating_add(u32::from(width).saturating_sub(1))
            / u32::from(width)
    }
}
...
mod tests {
    ...
    fn microsoft_host_text_buffer_backspace_right_side_vt_contract() {
        ...
        assert_eq!(cub, TextBufferPoint::new(5, 0));

        let mut state = HostWriteState::new(cub, attr);
        state.write_vt(&"x\u{8}y".encode_utf16().collect::<Vec<_>>()).unwrap();
    ...
    fn microsoft_host_text_buffer_get_word_boundaries_matches_all_source_vectors() {
        ...
        let start_cases = [
            WordCase { point:(0,0), selection:(0,0), accessibility:(0,0) },
            WordCase { point:(1,0), selection:(0,0), accessibility:(0,0) },
            WordCase { point:(3,0), selection:(0,0), accessibility:(0,0) },
            WordCase { point:(4,0), selection:(4,0), accessibility:(0,0) },
            WordCase { point:(5,0), selection:(5,0), accessibility:(5,0) },
            WordCase { point:(6,0), selection:(5,0), accessibility:(5,0) },
            WordCase { point:(20,0), selection:(10,0), accessibility:(5,0) },
            WordCase { point:(79,0), selection:(10,0), accessibility:(5,0) },
            WordCase { point:(0,1), selection:(0,1), accessibility:(5,0) },
            WordCase { point:(1,1), selection:(0,1), accessibility:(5,0) },
            WordCase { point:(2,1), selection:(2,1), accessibility:(2,1) },
            WordCase { point:(3,1), selection:(2,1), accessibility:(2,1) },
            WordCase { point:(5,1), selection:(2,1), accessibility:(2,1) },
            WordCase { point:(6,1), selection:(6,1), accessibility:(2,1) },
            WordCase { point:(7,1), selection:(6,1), accessibility:(2,1) },
            WordCase { point:(9,1), selection:(9,1), accessibility:(9,1) },
            WordCase { point:(10,1), selection:(9,1), accessibility:(9,1) },
            WordCase { point:(20,1), selection:(14,1), accessibility:(9,1) },
            WordCase { point:(79,1), selection:(14,1), accessibility:(9,1) },
        ];
        assert_word_cases(&buffer, &start_cases, true);

        let end_cases = [
            WordCase { point:(0,0), selection:(4,0), accessibility:(5,0) },
            WordCase { point:(1,0), selection:(4,0), accessibility:(5,0) },
            WordCase { point:(3,0), selection:(4,0), accessibility:(5,0) },
            WordCase { point:(4,0), selection:(5,0), accessibility:(5,0) },
            WordCase { point:(5,0), selection:(10,0), accessibility:(2,1) },
            WordCase { point:(6,0), selection:(10,0), accessibility:(2,1) },
            WordCase { point:(20,0), selection:(80,0), accessibility:(2,1) },
            WordCase { point:(79,0), selection:(80,0), accessibility:(2,1) },
            WordCase { point:(0,1), selection:(2,1), accessibility:(2,1) },
            WordCase { point:(1,1), selection:(2,1), accessibility:(2,1) },
            WordCase { point:(2,1), selection:(6,1), accessibility:(9,1) },
            WordCase { point:(3,1), selection:(6,1), accessibility:(9,1) },
            WordCase { point:(5,1), selection:(6,1), accessibility:(9,1) },
            WordCase { point:(6,1), selection:(9,1), accessibility:(9,1) },
            WordCase { point:(7,1), selection:(9,1), accessibility:(9,1) },
            WordCase { point:(9,1), selection:(14,1), accessibility:(80,9000) },
            WordCase { point:(10,1), selection:(14,1), accessibility:(80,9000) },
            WordCase { point:(20,1), selection:(80,1), accessibility:(80,9000) },
        ...
        let wrapped_start = [
            ...
            WordCase { point:(4,0), selection:(4,0), accessibility:(0,0) },
            WordCase { point:(5,0), selection:(5,0), accessibility:(5,0) },
            WordCase { point:(7,0), selection:(5,0), accessibility:(5,0) },
            WordCase { point:(4,1), selection:(5,0), accessibility:(5,0) },
            WordCase { point:(7,1), selection:(5,0), accessibility:(5,0) },
            WordCase { point:(9,1), selection:(8,1), accessibility:(5,0) },
            WordCase { point:(0,2), selection:(0,2), accessibility:(0,2) },
            WordCase { point:(9,2), selection:(0,2), accessibility:(0,2) },
            WordCase { point:(0,3), selection:(0,3), accessibility:(0,2) },
            WordCase { point:(7,3), selection:(6,3), accessibility:(0,2) },
            WordCase { point:(1,4), selection:(6,3), accessibility:(0,2) },
            WordCase { point:(4,4), selection:(4,4), accessibility:(4,4) },
            WordCase { point:(8,4), selection:(4,4), accessibility:(4,4) },
            WordCase { point:(0,5), selection:(4,4), accessibility:(4,4) },
            WordCase { point:(1,5), selection:(1,5), accessibility:(4,4) },
            WordCase { point:(9,5), selection:(2,5), accessibility:(2,5) },
        ];
        ...
        let wrapped_end = [
            ...
            WordCase { point:(1,0), selection:(4,0), accessibility:(5,0) },
            WordCase { point:(4,0), selection:(5,0), accessibility:(5,0) },
            WordCase { point:(5,0), selection:(8,1), accessibility:(0,2) },
            WordCase { point:(7,0), selection:(8,1), accessibility:(0,2) },
            WordCase { point:(4,1), selection:(8,1), accessibility:(0,2) },
            WordCase { point:(7,1), selection:(8,1), accessibility:(0,2) },
            WordCase { point:(9,1), selection:(10,1), accessibility:(0,2) },
            WordCase { point:(0,2), selection:(10,2), accessibility:(4,4) },
            WordCase { point:(9,2), selection:(10,2), accessibility:(4,4) },
            WordCase { point:(0,3), selection:(6,3), accessibility:(4,4) },
            WordCase { point:(7,3), selection:(4,4), accessibility:(4,4) },
            WordCase { point:(1,4), selection:(4,4), accessibility:(4,4) },
            WordCase { point:(4,4), selection:(1,5), accessibility:(2,5) },
            WordCase { point:(8,4), selection:(1,5), accessibility:(2,5) },
            WordCase { point:(0,5), selection:(1,5), accessibility:(2,5) },
            WordCase { point:(1,5), selection:(2,5), accessibility:(2,5) },
            WordCase { point:(4,5), selection:(10,5), accessibility:(10,5) },
    ...
    fn microsoft_host_text_buffer_get_text_rects_matches_block_and_line_vectors() {
        ...
        }
        {
            ...
            for (x, byte) in [(2,b'2'),(3,b'3'),(4,b'4'),(5,b'5'),(6,b'6'),(7,b'7')] {
                row.replace_glyph(x, 1, &[u16::from(byte)]).unwrap();
            }
    ...
    fn expected_unwrapped(block: bool, crlf: bool, trim: bool) -> String {
        ...
        if crlf {
            ...
            out.push_str("12345  345123  3");
        } else if block {
            out.push_str("12345  345123    3       ");
        } else {
            out.push_str("12345       345     123         3            ");
        }
        out
    }

    fn expected_wrapped(block: bool, crlf: bool, trim: bool) -> String {
        let mut out = String::new();
        if block {
            if crlf {
                if trim {
                ...
                out.push_str("1234567  345123");
            } else {
                out.push_str("1234567     345123            ");
            }
        } else if crlf {
            if trim {
                out.push_str("1234567\r\n  345123  \r\n");
            } else {
                out.push_str("1234567   \r\n  345123       \r\n     ");
            }
        } else if trim {
        ...
        for block in [false, true] {
            for crlf in [false, true] {
                for trim in [false, true] {
                    let unwrapped_request = PlainTextRequest {
                    ...
                    assert_eq!(
                        String::from_utf16_lossy(&get_plain_text(&unwrapped, unwrapped_request)),
                        expected_unwrapped(block, crlf, trim),
                        "unwrapped block={block} crlf={crlf} trim={trim}"
                    );

                    let wrapped_request = PlainTextRequest {
                        start: TextBufferPoint::new(0,0),
                    ...
                    assert_eq!(
                        String::from_utf16_lossy(&get_plain_text(&wrapped, wrapped_request)),
                        expected_wrapped(block, crlf, trim),
                        "wrapped block={block} crlf={crlf} trim={trim}"
                    );
                }
            }
        }
    ...
    fn reflow_fixture() -> (TextBuffer, Vec<CommandMark>) {
        ...
        for (y, len, wrap) in [
            (0,17,false),(1,22,false),(2,22,false),(3,13,false),
            (4,80,true),(5,10,false),(6,22,false),(7,16,false),
        ] {
    ...
    fn microsoft_host_text_buffer_reflow_prompt_regions_matches_all_dx_variants() {
        ...
        for (dx, command_end, output_y, third_y) in [
            ...
            let width = u16::try_from(80 + dx).unwrap();
            let mapped = reflow_command_marks(&buffer, &marks, width);
