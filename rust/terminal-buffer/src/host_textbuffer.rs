...
pub fn get_plain_text(buffer: &TextBuffer, request: PlainTextRequest) -> Vec<u16> {
    ...
    for y in start.y..=end.y {
        ...
        let (begin, limit) = if request.block_selection {
            ...
            (
                if y == start.y { start.x.min(buffer.width()) } else { 0 },
                if y == end.y { end.x.min(buffer.width()) } else { buffer.width() },
            )
        };
...
fn remap_point(
    ...
    let y = point.y.clamp(0, i32::from(buffer.height()).saturating_sub(1));
...
mod tests {
    ...
    fn assert_word_cases(buffer: &TextBuffer, cases: &[WordCase], start: bool) {
        ...
        for case in cases {
            for (mode, expected) in [
                ...
                assert_eq!(actual, p(expected), "word case {:?} mode {mode:?}", case.point);
            }
        }
    }
    ...
    fn microsoft_host_text_buffer_get_word_boundaries_matches_all_source_vectors() {
        ...
        let start_cases = [
            WordCase{point:(0,0),selection:(0,0),accessibility:(0,0)}, WordCase{point:(1,0),selection:(0,0),accessibility:(0,0)},
            WordCase{point:(3,0),selection:(0,0),accessibility:(0,0)}, WordCase{point:(4,0),selection:(4,0),accessibility:(0,0)},
            WordCase{point:(5,0),selection:(5,0),accessibility:(5,0)}, WordCase{point:(6,0),selection:(5,0),accessibility:(5,0)},
            WordCase{point:(20,0),selection:(10,0),accessibility:(5,0)}, WordCase{point:(79,0),selection:(10,0),accessibility:(5,0)},
            WordCase{point:(0,1),selection:(0,1),accessibility:(5,0)}, WordCase{point:(1,1),selection:(0,1),accessibility:(5,0)},
            WordCase{point:(2,1),selection:(2,1),accessibility:(2,1)}, WordCase{point:(3,1),selection:(2,1),accessibility:(2,1)},
            WordCase{point:(5,1),selection:(2,1),accessibility:(2,1)}, WordCase{point:(6,1),selection:(6,1),accessibility:(2,1)},
            WordCase{point:(7,1),selection:(6,1),accessibility:(2,1)}, WordCase{point:(9,1),selection:(9,1),accessibility:(9,1)},
            WordCase{point:(10,1),selection:(9,1),accessibility:(9,1)}, WordCase{point:(20,1),selection:(14,1),accessibility:(9,1)},
            WordCase{point:(79,1),selection:(14,1),accessibility:(9,1)},
        ];
        assert_word_cases(&buffer, &start_cases, true);

        let end_cases = [
            WordCase{point:(0,0),selection:(4,0),accessibility:(5,0)}, WordCase{point:(1,0),selection:(4,0),accessibility:(5,0)},
            WordCase{point:(3,0),selection:(4,0),accessibility:(5,0)}, WordCase{point:(4,0),selection:(5,0),accessibility:(5,0)},
            WordCase{point:(5,0),selection:(10,0),accessibility:(2,1)}, WordCase{point:(6,0),selection:(10,0),accessibility:(2,1)},
            WordCase{point:(20,0),selection:(80,0),accessibility:(2,1)}, WordCase{point:(79,0),selection:(80,0),accessibility:(2,1)},
            WordCase{point:(0,1),selection:(2,1),accessibility:(2,1)}, WordCase{point:(1,1),selection:(2,1),accessibility:(2,1)},
            WordCase{point:(2,1),selection:(6,1),accessibility:(9,1)}, WordCase{point:(3,1),selection:(6,1),accessibility:(9,1)},
            WordCase{point:(5,1),selection:(6,1),accessibility:(9,1)}, WordCase{point:(6,1),selection:(9,1),accessibility:(9,1)},
            WordCase{point:(7,1),selection:(9,1),accessibility:(9,1)}, WordCase{point:(9,1),selection:(14,1),accessibility:(80,9000)},
            WordCase{point:(10,1),selection:(14,1),accessibility:(80,9000)}, WordCase{point:(20,1),selection:(80,1),accessibility:(80,9000)},
            WordCase{point:(79,1),selection:(80,1),accessibility:(80,9000)},
        ];
        assert_word_cases(&buffer, &end_cases, false);

        let mut wrapped = TextBuffer::new(10, 6, attr).unwrap();
        for (y, text, wrap) in [
            (0,"this wordi",true),(1,"swrapped  ",false),(2,"notwrapped",false),
            (3,"spaces    ",true),(4,"    wrappe",true),(5,"d reachEOB",true),
        ] {
            write_ascii_row(&mut wrapped, y, text, wrap);
        }
        let wrapped_start = [
            WordCase{point:(0,0),selection:(0,0),accessibility:(0,0)}, WordCase{point:(1,0),selection:(0,0),accessibility:(0,0)},
            WordCase{point:(4,0),selection:(4,0),accessibility:(0,0)}, WordCase{point:(5,0),selection:(5,0),accessibility:(5,0)},
            WordCase{point:(7,0),selection:(5,0),accessibility:(5,0)}, WordCase{point:(4,1),selection:(5,0),accessibility:(5,0)},
            WordCase{point:(7,1),selection:(5,0),accessibility:(5,0)}, WordCase{point:(9,1),selection:(8,1),accessibility:(5,0)},
            WordCase{point:(0,2),selection:(0,2),accessibility:(0,2)}, WordCase{point:(9,2),selection:(0,2),accessibility:(0,2)},
            WordCase{point:(0,3),selection:(0,3),accessibility:(0,2)}, WordCase{point:(7,3),selection:(6,3),accessibility:(0,2)},
            WordCase{point:(1,4),selection:(6,3),accessibility:(0,2)}, WordCase{point:(4,4),selection:(4,4),accessibility:(4,4)},
            WordCase{point:(8,4),selection:(4,4),accessibility:(4,4)}, WordCase{point:(0,5),selection:(4,4),accessibility:(4,4)},
            WordCase{point:(1,5),selection:(1,5),accessibility:(4,4)}, WordCase{point:(9,5),selection:(2,5),accessibility:(2,5)},
        ];
        assert_word_cases(&wrapped, &wrapped_start, true);
        let wrapped_end = [
            WordCase{point:(0,0),selection:(4,0),accessibility:(5,0)}, WordCase{point:(1,0),selection:(4,0),accessibility:(5,0)},
            WordCase{point:(4,0),selection:(5,0),accessibility:(5,0)}, WordCase{point:(5,0),selection:(8,1),accessibility:(0,2)},
            WordCase{point:(7,0),selection:(8,1),accessibility:(0,2)}, WordCase{point:(4,1),selection:(8,1),accessibility:(0,2)},
            WordCase{point:(7,1),selection:(8,1),accessibility:(0,2)}, WordCase{point:(9,1),selection:(10,1),accessibility:(0,2)},
            WordCase{point:(0,2),selection:(10,2),accessibility:(4,4)}, WordCase{point:(9,2),selection:(10,2),accessibility:(4,4)},
            WordCase{point:(0,3),selection:(6,3),accessibility:(4,4)}, WordCase{point:(7,3),selection:(4,4),accessibility:(4,4)},
            WordCase{point:(1,4),selection:(4,4),accessibility:(4,4)}, WordCase{point:(4,4),selection:(1,5),accessibility:(2,5)},
            WordCase{point:(8,4),selection:(1,5),accessibility:(2,5)}, WordCase{point:(0,5),selection:(1,5),accessibility:(2,5)},
            WordCase{point:(1,5),selection:(2,5),accessibility:(2,5)}, WordCase{point:(4,5),selection:(10,5),accessibility:(10,5)},
            WordCase{point:(9,5),selection:(10,5),accessibility:(10,5)},
        ];
    ...
    fn microsoft_host_text_buffer_get_text_rects_matches_block_and_line_vectors() {
        ...
        let burrito = [0xd83c, 0xdf2f];
        {
            ...
            for (x,byte) in [(3,b'3'),(4,b'4'),(5,b'5'),(6,b'6')] { row.replace_glyph(x,1,&[u16::from(byte)]).unwrap(); }
            row.replace_glyph(7,2,&burrito).unwrap();
        }
        {
            let row = buffer.row_mut(2);
            row.replace_glyph(0,1,&[u16::from(b' ')]).unwrap(); row.replace_glyph(1,1,&[u16::from(b' ')]).unwrap();
            row.replace_glyph(2,2,&burrito).unwrap(); row.replace_glyph(4,1,&[u16::from(b'4')]).unwrap();
            row.replace_glyph(5,1,&[u16::from(b'5')]).unwrap(); row.replace_glyph(6,2,&burrito).unwrap();
        }
        {
            let row = buffer.row_mut(3);
            row.replace_glyph(0,2,&burrito).unwrap();
            for (x,byte) in [(2,b'2'),(3,b'3'),(4,b'4'),(5,b'5'),(6,b'6'),(7,b'7')] { row.replace_glyph(x,1,&[u16::from(byte)]).unwrap(); }
            row.replace_glyph(8,2,&burrito).unwrap();
        }
        let start = TextBufferPoint::new(1,0); let end = TextBufferPoint::new(8,4);
        assert_eq!(get_text_rects(&buffer,start,end,true), vec![
            InclusiveRect::new(1,0,8,0), InclusiveRect::new(1,1,9,1), InclusiveRect::new(1,2,8,2),
            InclusiveRect::new(0,3,8,3), InclusiveRect::new(1,4,8,4),
        ]);
        assert_eq!(get_text_rects(&buffer,start,end,false), vec![
            InclusiveRect::new(1,0,19,0), InclusiveRect::new(0,1,19,1), InclusiveRect::new(0,2,19,2),
            InclusiveRect::new(0,3,19,3), InclusiveRect::new(0,4,8,4),
        ]);
    }
    ...
    fn microsoft_host_text_buffer_get_plain_text_matches_complete_copy_request_matrix() {
        ...
        for (y,text) in [(0,"12345"),(1,"  345"),(2,"123  "),(3,"  3  ")] { write_ascii_row(&mut unwrapped,y,text,false); }
        let mut wrapped = TextBuffer::new(5,20,attr).unwrap();
        for (y,text,wrap) in [(0,"12345",true),(1,"67   ",false),(2,"  345",true),(3,"123  ",true),(4,"     ",false),(5,"     ",false)] {
            write_ascii_row(&mut wrapped,y,text,wrap);
        }
        for block in [false,true] { for crlf in [false,true] { for trim in [false,true] {
            let request = PlainTextRequest { start:TextBufferPoint::new(0,0), end:TextBufferPoint::new(5,4), block_selection:block, include_crlf:crlf, trim_trailing_whitespace:trim, format_wrapped_rows:false };
            assert_eq!(String::from_utf16_lossy(&get_plain_text(&unwrapped,request)), expected_unwrapped(block,crlf,trim));
            let request = PlainTextRequest { start:TextBufferPoint::new(0,0), end:TextBufferPoint::new(5,5), block_selection:block, include_crlf:crlf, trim_trailing_whitespace:trim, format_wrapped_rows:block };
            assert_eq!(String::from_utf16_lossy(&get_plain_text(&wrapped,request)), expected_wrapped(block,crlf,trim));
        }}}
    }

    fn write_prompt(state: &mut CommandRegionState) {
        state.command_finished(); state.prompt_start(); state.write_text("PWSH C:\\> "); state.command_start();
    }

    fn reflow_fixture() -> (TextBuffer, Vec<CommandMark>) {
        let attr = TextAttribute::default();
        let mut state = CommandRegionState::new(80);
        write_prompt(&mut state); state.write_text("Foo-bar"); state.command_end(); state.crlf();
        state.write_text("This is some text     "); state.crlf(); state.write_text("with varying amounts  "); state.crlf();
        state.write_text("of whitespace"); state.crlf(); write_prompt(&mut state); state.write_text(&"F".repeat(80));
        state.command_end(); state.crlf(); state.write_text("This is more text     "); state.crlf(); write_prompt(&mut state);
        state.write_text("yikes?"); state.command_end();
        let mut buffer = TextBuffer::new(80,10,attr).unwrap();
        for (y,len,wrap) in [(0,17,false),(1,22,false),(2,22,false),(3,13,false),(4,80,true),(5,10,false),(6,22,false),(7,16,false)] {
    ...
    fn microsoft_host_text_buffer_reflow_prompt_regions_matches_all_dx_variants() {
        ...
        assert_eq!(marks[0].start,Point::new(0,0)); assert_eq!(marks[0].end,Point::new(10,0));
        assert_eq!(marks[0].command_end,Some(Point::new(17,0))); assert_eq!(marks[0].output_end,Some(Point::new(13,3)));
        assert_eq!(marks[1].start,Point::new(0,4)); assert_eq!(marks[1].end,Point::new(10,4));
        assert_eq!(marks[1].command_end,Some(Point::new(10,5))); assert_eq!(marks[1].output_end,Some(Point::new(22,6)));
        for (dx,command_end,output_y,third_y) in [
            (-15,Point::new(25,5),6,7),(-1,Point::new(11,5),6,7),(0,Point::new(10,5),6,7),
            (1,Point::new(9,5),6,7),(15,Point::new(90,4),5,6),
        ] {
            let mapped = reflow_command_marks(&buffer,&marks,u16::try_from(80+dx).unwrap());
            assert_eq!(mapped[0].start,Point::new(0,0)); assert_eq!(mapped[0].end,Point::new(10,0));
            assert_eq!(mapped[0].command_end,Some(Point::new(17,0))); assert_eq!(mapped[0].output_end,Some(Point::new(13,3)));
            assert_eq!(mapped[1].start,Point::new(0,4)); assert_eq!(mapped[1].end,Point::new(10,4));
            assert_eq!(mapped[1].command_end,Some(command_end)); assert_eq!(mapped[1].output_end,Some(Point::new(22,output_y)));
            assert_eq!(mapped[2].start,Point::new(0,third_y)); assert_eq!(mapped[2].end,Point::new(10,third_y));
            assert!(mapped[2].command_end.is_some()); assert!(mapped[2].output_end.is_none());
        }
    }
}
