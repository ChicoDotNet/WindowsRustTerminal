...
mod tests {
    ...
    fn microsoft_text_buffer_append_rtf_text_contract() {
        ...
        append_rtf_text(&mut output, &"This is some Ascii \\ {}".encode_utf16().collect::<Vec<_>>());
        assert_eq!(output, "This is some Ascii \\\\ \\{\\}");

        output.clear();
        append_rtf_text(
            &mut output,
            &[0x004c, 0x006f, 0x0077, 0x0020, 0x0063, 0x006f, 0x0064, 0x0065, 0x0020, 0x0075, 0x006e, 0x0069, 0x0074, 0x0073, 0x003a, 0x0020, 0x00e1, 0x0020, 0x00e9, 0x0020, 0x00ed, 0x0020, 0x00f3, 0x0020, 0x00fa, 0x0020, 0x2b81, 0x0020, 0x2b82],
        );
        assert_eq!(output, "Low code units: \\u225? \\u233? \\u237? \\u243? \\u250? \\u11137? \\u11138?");

        output.clear();
        append_rtf_text(&mut output, &"High code units: ".encode_utf16().chain([0xa7b5, 0x0020, 0xa7b7]).collect::<Vec<_>>());
        assert_eq!(output, "High code units: \\u-22603? \\u-22601?");

        output.clear();
        append_rtf_text(&mut output, &"Surrogates: ".encode_utf16().chain([0xd83c, 0xdf66, 0x0020, 0xd83d, 0xdc7e, 0x0020, 0xd83d, 0xdc40]).collect::<Vec<_>>());
        assert_eq!(output, "Surrogates: \\u-10180?\\u-8346? \\u-10179?\\u-9090? \\u-10179?\\u-9152?");
    }
}
